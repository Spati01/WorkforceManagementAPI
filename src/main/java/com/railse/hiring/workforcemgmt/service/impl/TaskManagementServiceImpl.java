...
public class TaskManagementServiceImpl implements TaskManagementService {
    ...
    @Override
    public TaskManagementDto updatePriority(Long taskId, Priority newPriority) {
        TaskManagement task = taskRepository.findById(taskId)
                .orElseThrow(() -> new ResourceNotFoundException("Task not found with id: " + taskId));
        task.setPriority(newPriority);

        if (task.getActivityHistory() == null) task.setActivityHistory(new ArrayList<>());
        task.getActivityHistory().add("Priority changed to " + newPriority);
    ...
    public List<TaskManagementDto> createTasks(TaskCreateRequest createRequest) {
        ...
        for (TaskCreateRequest.RequestItem item : createRequest.getRequests()) {
            ...
            newTask.setActivityHistory(new ArrayList<>(List.of("Task created.")));
    ...
    public List<TaskManagementDto> updateTasks(UpdateTaskRequest updateRequest) {
        ...
        for (UpdateTaskRequest.RequestItem item : updateRequest.getRequests()) {
            ...
            if (item.getTaskStatus() != null) {
                ...
                task.getActivityHistory().add("Status changed to " + item.getTaskStatus());
            }
            if (item.getDescription() != null) {
                task.setDescription(item.getDescription());
                task.getActivityHistory().add("Description updated.");
            }
    ...
    public String assignByReference(AssignByReferenceRequest request) {
        ...
        for (Task taskType : applicableTasks) {
            ...
            for (TaskManagement oldTask : tasksOfType) {
                ...
                if (oldTask.getActivityHistory() == null) oldTask.setActivityHistory(new ArrayList<>());
                oldTask.getActivityHistory().add("Task cancelled due to reassignment.");
            ...
            newTask.setActivityHistory(new ArrayList<>(List.of("Task reassigned.")));
            newTask.setComments(new ArrayList<>());
